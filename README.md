...
**RFC-CAT 4649** 準拠の、内容証明郵便で送るためのひらがな Base32 双方向変換器。
...
(注: RFC番号 4649 は IETF 非公認の語呂合わせです。本物の Base32 は [RFC 4648](https://www.rfc-editor.org/rfc/rfc4648)。)
...
ユーザのプロンプトに全文掲載されている RFC-CAT 4649 を参照。要点:
...
¹ **誤植訂正**: 原典 RFC-CAT 4649 §4 では `4649` の値が `きそきけきそきす` と記載されていますが、これは §1.2 のパディング表 (4バイト→7文字+1パディング) と矛盾するため、本実装では正しい値 `きちむえけそけ。` を採用しています。
